...
if PYTEST_AVAILABLE:
    ...
    class TestGetHemcoSimulationTime:
        @pytest.fixture
        def hemco_file(self, tmp_path):
            """Fixture to create HEMCO time files for testing."""

            def _create_file(start_time, end_time):
                text = dedent(
                    f"""\
        ...
        def test_feb29_overlap(self, hemco_file):
            # A < 24h period that overlaps midnight
            p = hemco_file("2024-02-29 20:00:00", "2024-03-01 05:00:00")
        ...
        def test_feb29_12z(self, hemco_file):
            # A slice of one day
            p = hemco_file("2024-02-29 12:00:00", "2024-02-29 20:00:00")
            dates = get_hemco_simulation_time(p)
            assert dates == [
                datetime(2024, 2, 29, 12, 0),
            ]

        def test_multiday(self, hemco_file):
            # A multi-day period that ends on a time-of-day earlier than the start time
            p = hemco_file("2024-02-28 12:00:00", "2024-03-03 00:00:00")
            dates = get_hemco_simulation_time(p)
            assert dates == [
        ...
        def test_invalid_period(self, hemco_file):
            # START = END should raise an error
